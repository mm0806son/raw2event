...
def sample_levy(c, mu=0):
    ...
    # Lévy distribution with c = (alpha/sigma)^2; see references below.
    # https://en.wikipedia.org/wiki/Inverse_Gaussian_distribution
...
def sample_IG_torch(episilon, c, sigma):
    assert isinstance(episilon, torch.Tensor)
    assert isinstance(c, torch.Tensor)
    delta_t_tensor = torch.zeros_like(episilon, device=episilon.device)

    # The distribution depends on the drift: with drift it is Inverse
    # Gaussian, without drift it reduces to a Lévy distribution.
    # The block below handles the Lévy case (c == 0).
    zeros_mask = c == 0.0
...
def sample_truncated_normal(mean, scale, a, b):
    """[summary]
...
def event_generation(ep_on, ep_off, c, sigma, delta_vd_legacy, start_t, end_t, x=None, y=None):
    try:
        ...
        if torch.cuda.is_available() and torch.cuda.device_count() > 0:
            return event_generation_torch(ep_on, ep_off, c, sigma, delta_vd_legacy, start_t, end_t, x, y)
        else:
            return np_event_generation(ep_on, ep_off, c, sigma, delta_vd_legacy, start_t, end_t, x, y)
    except ImportError:
        return np_event_generation(ep_on, ep_off, c, sigma, delta_vd_legacy, start_t, end_t, x, y)


def event_generation_torch(ep_on, ep_off, c, sigma, delta_vd_legacy, start_t, end_t, x=None, y=None):
    ...
    # first-on probabilities
    # Decide whether an ON or OFF event fires first.
    # ref: stochastic-process textbook (first passage time of Brownian motion)
    ...
    u.uniform_()
    on_mask = u <= p_first_on  # mask of pixels firing the positive (OFF) event first

    ep_input = torch.where(on_mask, ep_on_real, ep_off_real).to(torch.float64)

    c_input = torch.where(on_mask, c, -1 * c)
    delta_t_step = sample_IG_torch(ep_input, c_input, sigma)  # sample $\Delta t$
    ...
    still_possible_events = delta_t_2_ends < 0  # before end_t: more events still possible

    # last_event_positions = torch.logical_not(still_possible_events)
    # Past end_t: compute the residual $\Delta V_d$ legacy term.
    ...
    if torch.sum(this_event_positions) > 0:
        ...
        y_next = events_y
        t_next = events_t
        e_t, e_x, e_y, e_p, e_delta_vd = event_generation_torch(
...
# ===================== numpy version =====================
def np_sample_non_c_zero(ep, c_in, sigma_in):
    pos_c_position = c_in > 0
    neg_c_position = c_in < 0
    X = np.empty_like(ep)
    X[pos_c_position] = np.random.randn(np.sum(pos_c_position))
    ep_neg = ep[neg_c_position]
    c_neg = c_in[neg_c_position]
    sigma_in_neg = sigma_in[neg_c_position]
    x_max_thres = -np.sqrt(-4 * ep_neg * c_neg / np.power(sigma_in_neg, 2.0))
    x_truncated = np_sample_truncated_normal(
        np.zeros_like(x_max_thres),
        np.ones_like(x_max_thres),
        -np.inf * np.ones_like(x_max_thres),
        x_max_thres)
    X[neg_c_position] = x_truncated
    mean = ep / c_in
    lambda_ig = np.power(ep / sigma_in, 2.0)
    scale = lambda_ig
    mu = mean / (2 * scale)
    Y = mean * X * X
    Z = 4 * scale * Y + Y * Y
    X = mean + mu * (Y - np.sqrt(Z))
    U = np.random.rand(*ep.shape)
    out = np.where(U > mean / (mean + X), mean * mean / X, X)
    return out

def np_sample_levy(c, mu=0):
    u = np.random.rand(*c.shape)
    ev = np.sqrt(2) * np_erfinv(1 - u)
    return c / np.power(ev, 2) + mu

def np_erfinv(x):
    return np.sqrt(2) * scipy.special.erfinv(x)

def np_sample_IG_numpy(episilon, c, sigma):
    delta_t_tensor = np.zeros_like(episilon)
    zeros_mask = c == 0.0
    ep_zeros_c = episilon[zeros_mask]
    sigma_zeros_c = sigma[zeros_mask]
    scale_levy = np.power(ep_zeros_c / sigma_zeros_c, 2.0)
    delta_t_c_zero = np_sample_levy(scale_levy)
    where_non_zeros = ~zeros_mask
    ep_non_zeros = episilon[where_non_zeros]
    c_non_zeros = c[where_non_zeros]
    sigma_non_zeros = sigma[where_non_zeros]
    delta_t_c_non_zero = np_sample_non_c_zero(
        ep_non_zeros, c_non_zeros, sigma_non_zeros)
    delta_t_tensor[zeros_mask] = delta_t_c_zero
    delta_t_tensor[where_non_zeros] = delta_t_c_non_zero
    return delta_t_tensor

def np_sample_truncated_normal(mean, scale, a, b):
    a_scaled = (a - mean) / scale
    b_scaled = (b - mean) / scale
    return truncnorm.rvs(a_scaled, b_scaled, loc=mean, scale=scale)

def np_event_generation(ep_on, ep_off, c, sigma, delta_vd_legacy, start_t, end_t, x=None, y=None):
    delta_vd_legacy = delta_vd_legacy.astype(np.float64)
    if x is None or y is None:
        ...
        y = yy.flatten()
    ep_on = ep_on.flatten()
    ep_off = ep_off.flatten()
    c = c.flatten()
    sigma = sigma.flatten()
    delta_vd_legacy = delta_vd_legacy.flatten()
    start_t = start_t.flatten().astype(np.float64)
    ep_on_real = ep_on - delta_vd_legacy
    ep_off_real = ep_off + delta_vd_legacy
    sigma_squared = sigma ** 2
    max_exp = 700
    min_exp = -700
    exp_input_B = np.clip(2 * c * ep_off_real / sigma_squared, a_min=min_exp, a_max=max_exp)
    exp_input_A = np.clip(-2 * c * ep_on_real / sigma_squared, a_min=min_exp, a_max=max_exp)
    exp_2uB = np.exp(exp_input_B)
    exp_2uA = np.exp(exp_input_A)
    denominator = exp_2uB - exp_2uA
    numerator = exp_2uB - 1
    eps = 1e-8
    denominator = np.where(np.abs(denominator) < eps, eps, denominator)
    p_first_on = numerator / denominator
    p_first_on = np.nan_to_num(p_first_on, nan=0.5, posinf=1.0, neginf=0.0)
    p_first_on[c == 0] = 0.5
    u = np.random.rand(*p_first_on.shape)
    on_mask = u <= p_first_on
    ep_input = np.where(on_mask, ep_on_real, ep_off_real)
    c_input = np.where(on_mask, c, -c)
    delta_t_step = np_sample_IG_numpy(ep_input, c_input, sigma)
    t_end_ideal = delta_t_step + start_t
    delta_t_2_ends = t_end_ideal - end_t
    delta_vd_legacy_new = np.copy(delta_vd_legacy)
    still_possible_events = delta_t_2_ends < 0
    last_event_positions = ~still_possible_events
    sign = on_mask.astype(np.float64) * 2 - 1
    ep_signed = sign * ep_input
    delta_vd_last = ep_signed * (end_t - start_t) / delta_t_step
    delta_vd_legacy_new[last_event_positions] += delta_vd_last[last_event_positions]
    this_event_positions = still_possible_events
    events_t = t_end_ideal[this_event_positions]
    events_x = x[this_event_positions]
    events_y = y[this_event_positions]
    events_p = on_mask[this_event_positions].astype(np.int32)
    # Recursive call for new events
    if np.any(this_event_positions):
        ep_on_next = ep_on[this_event_positions]
        ep_off_next = ep_off[this_event_positions]
        c_next = c[this_event_positions]
        sigma_next = sigma[this_event_positions]
        delta_vd_next = np.zeros_like(ep_on_next)
        x_next = x[this_event_positions]
        y_next = y[this_event_positions]
        t_next = t_end_ideal[this_event_positions]
        e_t, e_x, e_y, e_p, e_delta_vd = np_event_generation(
            ep_on_next, ep_off_next, c_next, sigma_next, delta_vd_next, t_next, end_t, x_next, y_next)

        events_t = np.concatenate([events_t, e_t])
        events_x = np.concatenate([events_x, e_x])
        events_y = np.concatenate([events_y, e_y])
        events_p = np.concatenate([events_p, e_p])

        delta_vd_legacy_new[this_event_positions] = e_delta_vd
    return events_t, events_x, events_y, events_p, delta_vd_legacy_new
